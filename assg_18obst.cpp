...
class OBST
{
    ...
    int w[SIZE][SIZE]; // Cost ‘c[i][j] of a tree having root ‘r[i][j]
    int r[SIZE][SIZE]; // Weight ‘w[i][j]’ of a tree having root
    //’r[i][j]’
    int c[SIZE][SIZE]; // represents root
...
public:
    ...
    void build_OBST()
    {
        ...
        for (m = 2; m <= n; m++)
        {
            ...
            {
                j = i + m;
                w[i][j] = w[i][j - 1] + p[j] + q[j];   //weight formula
                k = Min_Value(i, j);
                c[i][j] = w[i][j] + c[i][k - 1] + c[k][j];  //cost formula
                r[i][j] = k;    //root value
            }
        }
    }
